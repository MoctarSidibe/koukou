export enum ReferenceKey {
  STANDARD_MODULE = 'standard_module',
  DENSITY_WARN = 'density_warn_per_m2',
  DENSITY_CRITICAL = 'density_critical_per_m2',
  DEFAULT_SAC_KG = 'default_sac_kg',
  MORTALITY_EXPECTED_WEEK1_PCT = 'mortality_expected_week1_pct',
  MORTALITY_EXPECTED_GROWTH_WEEK_PCT = 'mortality_expected_growth_week_pct',
  MORTALITY_EXPECTED_CAP_PCT = 'mortality_expected_cap_pct',
  MORTALITY_DEV_WARN_PCT = 'mortality_dev_warn_pct',
  MORTALITY_DEV_CRITICAL_PCT = 'mortality_dev_critical_pct',
  WATER_DROP_WARN_PCT = 'water_drop_warn_pct',
  WATER_DROP_CRITICAL_PCT = 'water_drop_critical_pct',
  WATER_WINDOW_DAYS = 'water_window_days',
  WATER_PER_BIRD_L_DAY = 'water_per_bird_l_day',
  WATER_NORM_DEVIATION_WARN_PCT = 'water_norm_deviation_warn_pct',
  FEED_DROP_WARN_PCT = 'feed_drop_warn_pct',
  IPE_DEVIATION_WARN_PCT = 'ipe_deviation_warn_pct',
  GMQ_DEVIATION_WARN_PCT = 'gmq_deviation_warn_pct',
  VIDE_SANITAIRE_MIN_DAYS = 'vide_sanitaire_min_days',
  VIDE_SANITAIRE_MAX_DAYS = 'vide_sanitaire_max_days',
  AGE_GAP_MAX_WEEKS = 'age_gap_max_weeks',
  BUILDING_DENSITY_WARN = 'building_density_warn_per_m2',
  BUILDING_DENSITY_CRITICAL = 'building_density_critical_per_m2',
  PROPHYLAXIE_RETARD_WARN_DAYS = 'prophylaxie_retard_warn_days',
  CALENDAR_LEAD_DAYS = 'calendar_lead_days',
  FEED_STOCK_WARN_DAYS = 'feed_stock_warn_days',
  FEED_STOCK_CRITICAL_DAYS = 'feed_stock_critical_days',
  FEED_ORDER_TARGET_DAYS = 'feed_order_target_days',
  RENTABILITE_MARGE_MIN_PCT = 'rentabilite_marge_min_pct',
  VENTE_INVENDUS_DAYS = 'vente_invendu_days',
  EGG_STOCK_WARN_ALVEOLES = 'egg_stock_warn_alveoles',
  CUSTOMER_SEGMENT_TOP_MIN_VISITS = 'customer_segment_top_min_visits',
  CUSTOMER_SEGMENT_TOP_MIN_FCFA = 'customer_segment_top_min_fcfa',
  CUSTOMER_SEGMENT_REGULAR_MIN_VISITS = 'customer_segment_regular_min_visits',
  VENTE_AGE_MIN_DAYS = 'vente_age_min_days',
  VENTE_FCR_DEV_MAX_PCT = 'vente_fcr_dev_max_pct',
  REFORME_LAY_RATE_FALL_PCT = 'reforme_lay_rate_fall_pct',
}
