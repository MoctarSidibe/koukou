...
import { BookUp, ClipboardList, CreditCard, PackageOpen } from 'lucide-react-native';
...
type Mode = 'none' | 'quick' | 'daily' | 'sale' | 'feed';
...
export function QuickCaptureProvider({ children }: { children: React.ReactNode }) {
  ...
  const titles: Record<Exclude<Mode, 'none'>, { title: string; subtitle: string; icon: React.ReactNode }> = {
    ...
    sale: { title: 'Encaisser', subtitle: 'POS · espèces', icon: <CreditCard size={22} color={color.accent[600]} /> },
  ...
  return (
    <QuickCaptureContext.Provider value={api}>
      ...
      <Sheet
        ...
        {mode === 'quick' && (
          <QuickActions
            onPick={(key) => {
              ...
              if (key === 'vente') { setMode('sale'); return; }
        ...
        {mode === 'sale' && <SaleSheet initialBatchId={targetBatch} onClose={close} />}
